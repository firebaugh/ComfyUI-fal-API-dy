...
class Wan22VACEFun14bNode:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "task": (["depth", "pose", "inpainting", "outpainting", "reframe"], {"default": "depth"}),
                ...
                if isinstance(ref_images, torch.Tensor):
                    if ref_images.ndim == 4 and ref_images.shape[0] > 1:
                        for i in range(ref_images.shape[0]):
                            single_img = ref_images[i:i+1]
                            img_url = ImageUtils.upload_image(single_img)
                            if img_url:
                                ref_image_urls.append(img_url)
                    else:
                        img_url = ImageUtils.upload_image(ref_images)
                        if img_url:
                            ref_image_urls.append(img_url)
...
class DYWanFun22Node:
    ...
    def generate_video(
        ...
        try:
            ...
            if architecture == "vace" and strength_vace != 1.0:
